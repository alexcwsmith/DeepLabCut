...
def check_labeled_frames_analysis_likelihoods(
    ...
    for video in Videos:
        ...
        try:
            ...
            for bp in bodyparts:
                if return_all:
                    labeledFrameData.drop(
                        [(DLCscorer, bp, "x"), (DLCscorer, bp, "y")],
                        inplace=True,
                        axis=1,
                    )
                    lhdata = pd.concat(
                        [labeledFrameData], keys=[vname], names=["Video"], axis=1
                    )
                    ...
                    try:
                        dfcheck.columns = [vname+'_'+str(bp)]
                        cat = pd.concat([cat, dfcheck], axis=1)
                    except ValueError:
                        if verbose:
                            print(
                                "No low-confidence labeled frames detected for "
                                + str(vname)
                            )
                        continue
        except FileNotFoundError as e:
    ...
    if save:
        if return_all:
            if not os.path.exists(
                os.path.join(videofolder, "LabeledFrameAnalyzedData/")
            ):
                os.mkdir(os.path.join(videofolder, "LabeledFrameAnalyzedData/"))
            cat.to_csv(
                os.path.join(
                    videofolder,
                    "LabeledFrameAnalyzedData/"
                    + "Iteration"
                    + str(iteration)
                    + "_LabeledFrameAnalysisLikelihoods.csv",
                )
            )
        elif not return_all:
            ...
            cat.to_csv(
                os.path.join(
                    ...
                    "Iteration"
                    + str(iteration)
                    + "_FramesToDoubleCheckLabels_pcutoff"
                    + str(pcutoff_sn)
                    + ".csv",
                )
            )
