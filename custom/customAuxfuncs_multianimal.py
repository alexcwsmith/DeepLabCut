...
def form_default_inferencecfg(cfg):
    # load defaults
    inferencecfg = caux.read_plainconfig(
        os.path.join(caux.get_deeplabcut_path(), "inference_cfg.yaml")
    )
...
def read_inferencecfg(path_inference_config, cfg):
    ...
    try:
        inferencecfg = caux.read_plainconfig(str(path_inference_config))
    except FileNotFoundError:
        inferencecfg = form_default_inferencecfg(cfg)
        caux.write_plainconfig(
